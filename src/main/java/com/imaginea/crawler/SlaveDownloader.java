...
public class SlaveDownloader extends UntypedActor {
    ...
    public void onReceive(Object message) throws Exception {
        ...
        if (message instanceof DownloadWork) {
            ...
            List<String> linksPointingToMails = findLinksPointingToMails(url);
            System.out.println("For Month " + month + " "
                    + linksPointingToMails.size());
            for (String linkPointingToMail : linksPointingToMails) {
                MailInfo mailInfo;
                mailInfo = extractMailInfo(linkPointingToMail);
     ...
     *
     * @param url URL of Mail
     * @return MailInfo object which contains all the information about a mail
     * @throws IOException Exception Thrown if webClient cannot connect
     */
    public MailInfo extractMailInfo(String url) throws IOException {
        HtmlPage currentPage = webClient.getPage(url);
        List<HtmlElement> myelements = currentPage.getHead().getHtmlElementsByTagName("link");
        for(HtmlElement rem :myelements) {
            rem.remove();
        }
     ...
     *
     * @param mailInfo MailInfo containing information about a mail
     * @param month    Month for writing the mails to specific folders according to their
     *                 month.
     ...
     *
     * @param url URL of the page which contains links
     ...
     */
    public List<String> findLinksPointingToMails(String url)
            throws FailingHttpStatusCodeException, MalformedURLException, IOException {
        File file = new File(url);
        List<String> filesList = new LinkedList<>();
        for (File i : file.listFiles()) {
            filesList.add("file:///"+i.getAbsolutePath());
        }
        return filesList;
    }
}
